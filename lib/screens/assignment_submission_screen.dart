  ...
  void _submitAssignment() {
    ...
    setState(() {
      assignment = assignment.copyWith(
        isSubmitted: true,
        submissionText: _textController.text.isNotEmpty ? _textController.text : null,
        fileUrl: _selectedFileName, 
      );
      
      // Update mock data for immediate UI feedback
      final index = MockData.assignments.indexWhere((a) => a.id == widget.assignmentId);
      if (index != -1) {
          ...
          children: [
            ...
            Card(
              ...
              shape: RoundedRectangleBorder(
                ...
                side: BorderSide(color: assignment.isSubmitted ? Colors.green : Colors.orange),
              ),
              color: assignment.isSubmitted ? Colors.green.shade50 : Colors.orange.shade50,
              child: ListTile(
                leading: Icon(
                  assignment.isSubmitted ? Icons.check_circle : Icons.pending_actions,
                  color: assignment.isSubmitted ? Colors.green : Colors.orange,
                ),
                title: Text('Submission Status: ${assignment.isSubmitted ? 'Submitted' : 'Not Attempted'}'),
                subtitle: assignment.grade != null 
                    ? Text('Grade: ${assignment.grade} / ${assignment.maxGrade}')
                    : null,
              ),
            ),
            ...
            if (!assignment.isSubmitted || assignment.grade == null) ...[
              const Text('Online Text:', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              TextField(
